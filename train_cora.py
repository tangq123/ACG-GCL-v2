...
def train(args):
    ...
    for epoch in range(args.epoch):
        ...
        # aug_adj = aug_adj + torch.eye(n_samples).to(args.device)
    ...
    # # load dataset
    # if args.name not in ['cora', 'citeseer', 'pubmed','computers', 'photo','cs','physics']:
    #     model = model.cpu()
    ...
    # aug_adj = aug_adj + torch.eye(n_nodes).to(args.device)
    aug_adj = normalize_adj_torch(aug_adj, self_loop=True)
